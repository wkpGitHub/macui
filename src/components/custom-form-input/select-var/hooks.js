      ...
      return state.isShow && <CipDialog title={'选择变量'} model-value={true} onUpdate:modelValue={() => { state.isShow = false }} onConfirm={onConfirm}>
        <CipTabs model-value={0}>
          <CipTabPane label='全局变量' name={0}>
            <ul>
              {parentState.rootNode.globalValue.map(item => <li style="cursor: pointer; margin: 4px" onClick={() => selectVar(item, 'global.')}>{item.name} <ElTag>{dateTypeMap[item.dataType].label}</ElTag></li>)}
            </ul>
          </CipTabPane>
          ...
          <CipTabPane label='服务入参' name={2}>
            <ul>
              {(parentState.rootNode.inputParams || []).map(item => <li style="cursor: pointer; margin: 4px" onClick={() => selectVar(item, 'inputParams.')}>{item.name} <ElTag>{dateTypeMap[item.dataType].label}</ElTag></li>)}
            </ul>
          </CipTabPane>
          <CipTabPane label='服务出参' name={3}>
            <ul>
              {(parentState.rootNode.outParams || []).map(item => <li style="cursor: pointer; margin: 4px" onClick={() => selectVar(item, 'outParams.')}>{item.name} <ElTag>{dateTypeMap[item.dataType].label}</ElTag></li>)}
            </ul>
          </CipTabPane>
